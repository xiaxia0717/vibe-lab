...
def main():
    ...
    # ---------------- 2 虚拟摄像头 ----------------
    print("\n[2/4] 虚拟摄像头（需要 OBS）")
    obs_dir = Path(r"C:\Program Files\obs-studio")
    if obs_dir.exists():
        print(f"{OK} OBS Studio 已安装")
    else:
    ...
    try:
        ...
        if obs_dir.exists():
            try:
                cam = pyvirtualcam.Camera(width=640, height=480, fps=10)
    ...
    if problems:
        ...
        print("\n  使用流程：")
        print("    1. 手机插数据线")
        print("    2. 双击 phone-as-webcam\\启动.bat")
        print("    3. 手机打开「手机摄像头」App，点启动")
    print("=" * 62)
